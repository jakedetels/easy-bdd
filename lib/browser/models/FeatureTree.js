...
import {assert, expect} from '../assertions';
...
fn.getFeatureTree = function getFeatureTree (featureTreeName) {  
  var featureTree = _.findWhere(this.featureTrees, {name: featureTreeName});
...
FeatureTree.prototype.init = function init() {
  ...
  stepLoaders.forEach( loadStepsFn => {
    ...
    utils.callWith(this, loadStepsFn, {
      assert: assert,
      expect: expect,
      sinon: require('sinon')
    });
...
function registerStepFunction(type, step, fn, options) {
  var stepFunction = new StepFunction(step, fn, options);
...
FeatureTree.prototype.After = function(...args) {
...
FeatureTree.prototype.prepareWorld = function() {
  ...
  this.World = World.create(this.worldPrototype, this.stepFunctions, ParentWorld);
};
